...
if address_input:
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            document.dispatchEvent(new CustomEvent("geo_success", {
                detail: {lat: position.coords.latitude, lon: position.coords.longitude}
            }));
        },
        (err) => {
            document.dispatchEvent(new CustomEvent("geo_error", {detail: err.message}));
        }
    )
    """
    st.components.v1.html(f"<script>{js_code}</script>", height=0)
    st.info("ℹ️ 手機或瀏覽器定位中，如果無法定位，將使用預設位置。")
    location_success = True  # 預設位置也算成功
...
try:
    ...
    if "Latitude" not in df.columns or "Longitude" not in df.columns:
        st.error("❌ 資料檔缺少 Latitude 或 Longitude 欄位")
        st.stop()
